...
def bfs(bs, rows, cols):
    ...
    visited = set()
    visited.add((start[0], start[1] % cycle))

    while Q:
        v = Q.popleft()
        position = v[0]

        if v[0] == (rows-1, cols-1):
            return v
        else:
            if position != position1:
                ...
                for x, y in possibilities:
                    ...
                    if (
                        x >= 0 and x <= rows-1 and
                        y >= 0 and y <= cols-1 and
                        (x, y) not in bzds_next_loc and
                ...
                possibilities = [(position[0]+1, position[1]), position]
...
result = bfs(blizzards, r, c1)
print(r, c1)
print(result[0], result[1]+1, "steps")
